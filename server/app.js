...
function beforeRequest(req, res, next) {
  ...
  if(req.path == '/activate' || req.path == '/activateMobile' || req.path == '/activateChange'){
    res.sendfile(__dirname + '../client/app/activation-page/activation.html');
  } else if (req.path == '/api/users/login' || req.cookies && req.cookies.isAuthenticated === "true"){
    next();
  } else {
    console.log('sending landing');
    res.sendfile('client/landing-3/index.html');
  }
}
...
console.log("root join " + path.join(root, 'client'));
...
app.get('/', function(req, res) {
  res.sendfile(__dirname + '../client/app/index/index.html');
});
